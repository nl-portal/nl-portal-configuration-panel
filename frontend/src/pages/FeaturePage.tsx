...
const FeaturePage = () => {
  ...
  const handleSubmit = () => {
    if (isDirty) {
      setIsDirty(false);
      const configurationProperties = toProperties(
        modifiedConfig,
        feature?.featureConfigurationPrefix,
      );
  ...
  useEffect(() => {
    ...
    storeConfigurations,
    storeConfigurationsError,
    deleteConfigurationsError,
  ]);

  useEffect(() => {
    if (featureConfigurations) {
      const prefillData = parseProperties(
    ...
    <PageGrid>
      <PageHeader>
        ...
        <div className={styles["feature-config__header"]}>
          <Heading2>
            <FormattedMessage id={"features." + featureId} />
          </Heading2>
        </div>
      </PageHeader>
      <div className={styles["feature-config__content"]}>
      ...
      <div>
        <div className={styles["feature-config__buttons"]}>
          ...
          >
            <FormattedMessage
              id={isDirty ? "action.cancel" : "action.back"}
            ></FormattedMessage>
          </Button>
        </div>
      </div>
