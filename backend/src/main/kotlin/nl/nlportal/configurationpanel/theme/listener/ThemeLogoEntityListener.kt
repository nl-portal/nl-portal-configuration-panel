    ...
    fun storeLogoAsConfiguration(themeLogo: ThemeLogo) {
        ...
        val configurationProperty =
            ConfigurationProperty(
                propertyKey = THEME_LOGO_CONFIGURATION_PROPERTY_KEY,
                propertyValue = themeLogo.asBase64DataUri(),
                application = themeLogo.application,
                profile = themeLogo.profile,
                label = themeLogo.label,
            ).also {
                logger.debug { "Attempting to save Theme Logo Configuration for Application: ${it.application}" }
            }

        configurationsService.saveConfigurationProperty(configurationProperty)?.also {
            logger.debug { "Saved Theme Logo Configuration for Application: ${it.application}" }
        }
    }
